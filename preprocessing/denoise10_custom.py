...
def smooth(path):
    ...
    y_abn = y
    dif_error = 2
    for i in range(len(pre_dif)-4):    ################################ 1. ################################
        if i==len(pre_dif):
            abnormal[i]=0
    ...
    mis5_X = []
    mis5_y = []
    for i in range(len(mis5)-12):    ################################ 5. 8. ################################
    ...
    df['x'][:len(mis5_X)] = mis5_X    ################################ 6. ################################
    df['y'][:len(mis5_y)] = mis5_y    ################################ 7. ################################

    outputName = os.path.basename(path)
    out_dir = r"C:\Users\user\Badminton\src\TrackNetV2-pytorch-option\trajectorylatest_csv\denoise"
    os.makedirs(out_dir, exist_ok=True)
    df.to_csv(os.path.join(out_dir, outputName[:-4] + "_denoise.csv"), index=False)

    os.makedirs(out_dir, exist_ok=True)
    df.to_csv(os.path.join(out_dir, outputName[:-4] + "_denoise.csv"), index=False)    ################################ 10. ################################

if __name__ == '__main__':
    predictPath = r"C:\Users\user\Badminton\src\TrackNetV2-pytorch-option\runs\detect"    ################################ 9. ################################
    predictList = os.listdir(predictPath)
    predictList = sorted(predictList)
    print(predictList)

for pi in predictList:
    # only process prediction CSVs
    if (not pi.lower().endswith(".csv")) or ("predict" not in pi.lower()):
        continue

    path = os.path.join(predictPath, pi)
    print("Denoising:", path)
    smooth(path)
