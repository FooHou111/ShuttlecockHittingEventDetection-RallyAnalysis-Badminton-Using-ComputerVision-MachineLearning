...
def main():
    ...
    csvPath = r"C:\Users\user\Badminton\src\postprocess\golfdb_G3_fold5_iter3000_val_test_X.csv"    ################################ 3. csv-path ################################
    ...
    ################
    """
    videoNames = df['VideoName'].tolist()
    for i in range(len(videoNames)-1):
        ...
        if videoNames[i+1] == o:
            ...
            winnerSet.remove(df['Hitter'][i])
            df['Winner'][i] = list(winnerSet)[0]
            o = videoNames[i+1]
    """
        
    df.to_csv('golfdb_G3_fold5_iter3000_val_test_hitter_vote.csv', index=False)    ################################ 5. csv-name ################################
    ...
    df.to_csv('golfdb_G3_fold5_iter3000_val_test_hitter_mean.csv', index=False)    ################################ 6. csv-name ################################
