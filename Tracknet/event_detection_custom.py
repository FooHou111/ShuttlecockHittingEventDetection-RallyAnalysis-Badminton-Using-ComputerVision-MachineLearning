...
denoise_rally_Path = r"C:\Users\user\Badminton\src\TrackNetV2-pytorch-option\trajectorylatest_csv\denoise"    ################################ 1. folder-path ################################
denoise_rally_List = os.listdir(denoise_rally_Path)
denoise_rally_List = sorted(denoise_rally_List)
#print(denoise_rally_List)

for drf in denoise_rally_List:
    filename = os.path.join(denoise_rally_Path, drf)
    ...
    peaks, properties = find_peaks(y, prominence=5)#distance=10) #y是吃非0,所以index有跳
    ...
    for i in range(len(peaks)-1):
        ...
        upper=[]
        plt.plot(z[start:end],y[start:end]*-1,'-')#plt.plot(frames[start:end],realy[start:end],'-')
    ...
    event_dir = r"C:\Users\user\Badminton\src\TrackNetV2-pytorch-option\10-10Gray\event"
    os.makedirs(event_dir, exist_ok=True)
    with open(os.path.join(event_dir, outputName[:-4] + "_event.csv"), 'w', newline='') as csvfile1:        ############################### 2. save-path ################################
