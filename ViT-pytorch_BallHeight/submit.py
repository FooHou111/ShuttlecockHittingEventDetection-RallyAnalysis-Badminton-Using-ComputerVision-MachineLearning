...
def setup(args):
    ...
    state = ckpt.get("state_dict", ckpt)   # if checkpoint is wrapped
...
def main():
    ...
    parser.add_argument("--test_dir", default= r"C:\Users\user\Badminton\src\postprocess\HitFrame",
                        help="Where to do the inference.") 
    parser.add_argument("--dataset", default='',
                        help="What kind of dataset to do the inference.")

    parser.add_argument("--num_classes", default=2, type=int,    ################################ 1. class-num ################################
    ...
    models = models[1:-1]
    modelTypes = [m.strip().strip("'").strip('"') for m in models.split(',')]

    cp = args.checkpoint
    cp = cp[1:-1]
    cpFiles = [c.strip().strip("'").strip('"') for c in cp.split(',')]
    ...
    num2cls = dict()
    num2cls = {0:1,1:2}    ################################ 2. all class ################################


    
    print(num2cls)

    imgNum = len(os.listdir(os.path.join(args.test_dir, '1')))   # number of all testing data
    #print(imgNum)]
    ...
    csvPath1 = r"C:\Users\user\Badminton\src\ViT-pytorch_Backhand\golfdb_G3_fold5_iter3000_val_test_hitter_vote_roundhead_vote_backhand_vote.csv"    ################################ 3. csv-path ################################
    csvPath2 = r"C:\Users\user\Badminton\src\ViT-pytorch_Backhand\golfdb_G3_fold5_iter3000_val_test_hitter_mean_roundhead_mean_backhand_mean.csv"    ################################ 3. csv-path ################################
    ...
    df1['BallHeight'] = Hitter    ################################ 4. attribute ################################
    df1.to_csv('golfdb_G3_fold5_iter3000_val_test_hitter_vote_roundhead_vote_backhand_vote_ballheight_vote.csv', index=False)    ################################ 5. csv-name ################################
    ...
    df2['BallHeight'] = Hitter    ################################ 4. attribute ################################
    df2.to_csv('golfdb_G3_fold5_iter3000_val_test_hitter_mean_roundhead_mean_backhand_mean_ballheight_mean.csv', index=False)    ################################ 5. csv-name ################################
